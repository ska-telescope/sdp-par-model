...
class ContainerTests(unittest.TestCase):
    def test_bldep(self):
        ...
        self.assertEqual(
            BLDep({"b": b, "c": b2}, b * b2, defaults={"b": 20})(c=2000), 40000
        )
    ...
    def test_sum(self):
        ...
        self.assertEqual(
            bs.eval_sum(symbins).doit(), Sum(Bcount(i) * B(i), (i, 1, n))
        )
        self.assertEqual(BLDep(b, 1).eval_sum(symbins).doit(), n)
        self.assertEqual(
            blsum(b, 1).eval_sum(symbins).doit(), Sum(Bcount(i), (i, 1, n))
        )
        ...
        self.assertEqual(
            blsum(b, free * b).eval_sum(symbins).doit(),
            free * Sum(Bcount(i) * B(i), (i, 1, n)),
        )
        self.assertEqual(
            BLDep(b, free).subs({free: b}).eval_sum(symbins).doit(),
            Sum(B(i), (i, 1, n)),
        )
    ...
    def test_symbolify(self):
        ...
        self.assertEqual(
            BLDep(b, b).eval_sum(tp.baseline_bins), Sum(Bmax, (ib, 1, Nbl))
        )
        self.assertEqual(
            blsum(b, 3 * b).eval_sum(tp.baseline_bins),
            3 * Sum(Bmax, (ib, 1, Nbl)),
        )
