...
from sympy import Max, Min, Symbol, floor, log, sign

from sdp_par_model.config import PipelineConfig
from sdp_par_model.evaluate import *
from sdp_par_model.parameters.container import ParameterContainer
from sdp_par_model.parameters.definitions import Bands, Products, Telescopes
...
class EvaluateTests(unittest.TestCase):
    def test_minimise_expr(self):
        ...
        self.assertDictEqual(
            test_xyz((xi - 2) ** 2 + (yi - 3) ** 2), {"xi": 2, "yi": 3}
        )

    def test_minimise_parameters(self):
        unopt_cfg = PipelineConfig(
        ...
        self.assertAlmostEqual(
            float(opt_tp.Rflop), float(opt_tp2.Rflop), delta=10
        )
        self.assertAlmostEqual(
            float(opt_tp.Nfacet), float(opt_tp2.Nfacet), places=4
        )
        self.assertAlmostEqual(
            float(opt_tp.products[Products.DFT]["Rflop"]),
            float(opt_tp2.products[Products.DFT]["Rflop"]),
            places=3,
        )
